...
class MeiliClient:
    ...
    def create_index_repositories(self):
        ...
        index.update_filterable_attributes(
            ["id", "language", "stars", "archived", "created_at", "pushed_at"]
        )
        index.update_sortable_attributes(["stars", "stars_per_day"])
    ...
    def save_repositories(self, repos: list[Repository]):
        ...
        for repo in repos:
            ...
            if repo.description and len(repo.description) > 1024:
                print(
                    f"Low quality repo: {repo.full_name}, description length: {len(repo.description)}"
                )
                continue
            ...
            docs.append(
                {
                    ...
                    "stars_per_day": repo.stargazers_count
                    / ((today - repo.created_at).days + 1),
                    "topics": repo.topics,
    ...
    def search_repositories(
        ...
        ranking_score_threshold: float = 0.2,
    ) -> list[dict]:
        index = self.client.index(self.INDEX_REPOSITORIES)
